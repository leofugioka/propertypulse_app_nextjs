...
const PropertyEditForm = ({ property }: Readonly<PropertyEditFormProps>) => {
  const { id, owner } = property;
  const [formState, action, isPending] = useActionState(actions.updateProperty.bind(null, { id, owner }), { errors: {} });
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-4 bg-blue-50 p-4">
        <div className="block text-gray-700 font-bold mb-2">Location</div>
      ...
      <div className="mb-4">
        <div className="block text-gray-700 font-bold mb-2">Amenities</div>
      ...
      <div className="mb-4 bg-blue-50 p-4">
        <div className="block text-gray-700 font-bold mb-2">Rates (Leave blank if not applicable)</div>
        <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
          <div className="flex items-center">
            <label htmlFor="weekly_rate" className="mr-2">
              Weekly
            </label>
            <input type="number" id="weekly_rate" name="rates.weekly" className="border rounded w-full py-2 px-3" defaultValue={property.rates.weekly ?? 0} />
          </div>
          <div className="flex items-center">
            <label htmlFor="monthly_rate" className="mr-2">
              Monthly
            </label>
            <input type="number" id="monthly_rate" name="rates.monthly" className="border rounded w-full py-2 px-3" defaultValue={property.rates.monthly ?? 0} />
          </div>
          <div className="flex items-center">
            <label htmlFor="nightly_rate" className="mr-2">
              Nightly
            </label>
            <input type="number" id="nightly_rate" name="rates.nightly" className="border rounded w-full py-2 px-3" defaultValue={property.rates.nightly ?? 0} />
          </div>
        </div>
      </div>
      ...
      {/* TODO implement edit images later */}
      {/* <div className="mb-4">
        <label htmlFor="images" className="block text-gray-700 font-bold mb-2">
          Images (Select up to 4 images)
        </label>
        <input type="file" id="images" name="images" className="border rounded w-full py-2 px-3" accept="image/*" multiple required translate="no" />
      </div> */}
      ...
      <div>
        <button
          ...
          type="submit"
        >
          {isPending ? "Loading..." : "Update Property"}
        </button>
      </div>
    </form>
  );
